    ...
    <div style={{minHeight:"100dvh",background:"#0a0a0a",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif",color:"#F6F6F4",padding:"24px min(8vw,32px)"}}>
      ...
      <div style={{width:"100%",maxWidth:400,animation:`fadeUp .8s ${S} both`}}>
        ...
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:20}}>
          <div>
            <label style={{fontSize:10,color:"rgba(246,246,244,.35)",letterSpacing:2.5,textTransform:"uppercase",fontWeight:600,display:"block",marginBottom:10}}>Data di nascita</label>
            <input type="date" value={birthDate} onChange={e=>setBirthDate(e.target.value)} required style={{...inputStyle,colorScheme:"dark"}}/>
          </div>
          <div>
            <label style={{fontSize:10,color:"rgba(246,246,244,.35)",letterSpacing:2.5,textTransform:"uppercase",fontWeight:600,display:"block",marginBottom:10}}>Ora di nascita</label>
            ...
            <label style={{display:"flex",alignItems:"center",gap:10,marginTop:10,cursor:"pointer",padding:"4px 0"}}>
              <input type="checkbox" checked={unknownTime} onChange={e=>{setUnknownTime(e.target.checked);if(e.target.checked)setBirthTime('')}} style={{accentColor:"#F4C430",width:16,height:16}}/>
              <span style={{fontSize:14,color:"rgba(246,246,244,.35)"}}>Non conosco l'ora di nascita</span>
            </label>
          </div>
          <div>
            <label style={{fontSize:10,color:"rgba(246,246,244,.35)",letterSpacing:2.5,textTransform:"uppercase",fontWeight:600,display:"block",marginBottom:10}}>Luogo di nascita</label>
